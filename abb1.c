...
int _comparar_claves(tipoclave cl1, tipoclave cl2) {
    ...
    return strcmp(cl1,cl2)==0 ? 0 : strcmp(cl1,cl2)==1 ? 1 : -1;
}

/* Destrucción elemento*/
...
tipoelem* buscar_nodo(abb A, tipoclave cl) {
    ...
    if (comp == 0) { // cl == A->info
        ...
        return buscar_nodo(A->izq, cl);
    } else { // cl > A->info
        return buscar_nodo(A->der, cl);
    }
}
...
tipoelem* insertar(abb *A, tipoelem E) {
    if (es_vacio(*A)) {
